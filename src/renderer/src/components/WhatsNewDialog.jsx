...
const KIND_META = {
  new: { Icon: Gift, label: 'New', colorCls: 'text-emerald-400' },
  improved: { Icon: Wrench, label: 'Improved', colorCls: 'text-sky-400' },
  fixed: { Icon: Bug, label: 'Fixed', colorCls: 'text-amber-400' },
  removed: { Icon: CircleMinus, label: 'Removed', colorCls: 'text-rose-400' },
}
  ...
  return (
    ...
    >
      <DialogContent className="flex flex-col max-h-[85vh] max-w-lg sm:max-w-lg gap-3">
        <DialogHeader>
          <DialogTitle>What&apos;s new{version ? ` in v${version}` : ''}</DialogTitle>
          <DialogDescription>Here&apos;s what&apos;s changed since your last launch.</DialogDescription>
        </DialogHeader>
            ...
            <section key={entry.version} className="space-y-2.5">
              {!single && (
                <div className="flex items-baseline gap-2 pb-1 border-b border-border/60">
                  <span className="text-sm font-semibold text-accent-blue">v{entry.version}</span>
                  <span className={CLARIFY}>{entry.date}</span>
                </div>
              )}
  ...
  return (
    <li className="flex gap-2.5">
      ...
      <div className="min-w-0 flex-1">
        <div className="text-sm font-medium text-text-primary select-text">{note.title}</div>
        <div className={`${CLARIFY} leading-snug select-text`}>{note.body}</div>
      </div>
    </li>
  )
}
