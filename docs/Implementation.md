...
- **Library**: Browse installed packages as a gallery (compact/detailed cards) or table. Filter by label, Status, Enabled, and labels. **Archived** is a Status shelf (not an Enabled value), separate from "my library by activity"; gated until an archive dir exists so a destructive-adjacent action never appears with a Settings detour. Other Status facets exclude archived so a hoard doesn't pollute Installed/Missing/Broken. Select a package for the detail panel (content, deps, dependents, labels, uninstall/promote/demote/disable/archive/install-from-archive/delete-from-disk/…). Context menu and bulk actions include Archive / Install from archive when archive dirs exist; leaving the archive is **Delete from disk** (permanent `.var` deletion, with a confirm that calls out local-only packages). Install is framed as a primary _install_ (not muted enable) because it downloads. Cards show a "no preset" checkmark once appearance presets have been extracted.
...
**Rules of thumb:** ≤2 sizes and ≤3 colors per block. Size steps between body and clarification; color steps between clarification and aside. Never put body/clarification on tertiary/aside. Never opacity-modify theme text tokens. Do not invent `text-[Npx]` for prose outside the recipes (retired: 9 / 13 / 15 / 17 / 22px; `text-[10px]` chrome/chips only; 16px headings spell `text-[16px]`, never `text-base` — `--color-base` shadows it into a color utility). Photo overlays may use `text-white/NN`. Never override `DialogDescription` / `AlertDialogDescription` size or color. App code uses `text-text-*`; shadcn aliases stay inside `components/ui/*`.
...
| --------------------- | ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `useLibraryStore`     | Local packages, filters, detail    | `packages[]`, filter state, `selection[]` + `selectionAnchor` / `selectionLead` + `selectedDetail`, `viewMode`, `selectedLabelIds`, `missingDeps`, `updateCheckResults` |
...
| `useContentStore`     | Content items, filters, detail     | `contents[]`, filter state, `selection[]` + `selectionAnchor` / `selectionLead` + `selectedItem` / `selectedPackage`, `selectedLabelIds`, `viewMode`, `expandedByType`  |
...
| `useHintsStore`       | One-shot UI tips (persisted)       | `multiSelectTipDone` — Library/Content multi-select tip; localStorage via `persistViewState`                                                                            |
| `useStatusStore`      | Status bar stats and scan progress | `stats{}`, `scan{}`                                                                                                                                                     |
| `useToastStore`       | Notification toasts                | `toasts[]`, `add()`, `dismiss()`                                                                                                                                        |

**Selection model** (Library + Content): `selection` is one ordered array of keys — filenames or content ids — and its length decides the mode. One pick is a single selection (detail panel); more is bulk (gallery + bulk toolbar). Two positions ride with it: `selectionAnchor` (Shift-range pivot) and `selectionLead` (keyboard/mouse focus; may sit on an unselected item after Ctrl-nav or after arrowing in bulk). The lead always moves; the selection follows the lead only while single — so bare Arrow/Home/End single-selects in the detail-browsing loop, and moves the lead without destroying a multi-selection. `setSelection(keyOrArray, { anchor, lead })` is the only writer, so the promote-to-single rule and its detail fetch live in one place; `selectedDetail` / `selectedItem` / `selectedPackage` are caches it fills, not selection state. The array is never emptied by the mutators — ctrl-deselecting the last pick is a no-op, and the views' auto-select refills after a deliberate `clearSelection`. Shared mutators and readers live in `stores/selection.js` (`isBulk`, `soleSelected`, `selectionMutators`). A one-shot dismissible tip in the idle toolbar (`SelectionHint` + `useHintsStore`) advertises Ctrl/Shift-click; the bulk toolbar keeps a muted `?` cheatsheet.
